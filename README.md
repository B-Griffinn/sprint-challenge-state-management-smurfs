...
Context API solves the propblem of Prop Drilling. Using Context.Provider to wrap our APP is a great way to pass down properties to any component within our APP without drilling.

- [X] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
Actions: The only way (in redux) to modify the APP state --> dispatch actions.
Reducers: Reducers are pure functions in that they will always give the same output if given the same input. Reducers are responsible for calculating the new version of state.
Store: The store is a single JS object that represents all changes within my APP. The store is where our redux app is stored.

- [X] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state is the current state of our entire application and is best used with Contexr API and Redux.
Componnet State is an individual components state, when drilling is not shared with any other component. Comp State would be useful in a very simple todo application.

- [X] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
Redux Thunk is a middleware that allows us to handle async operation INSIDE our action creators.

- [X] What is your favorite state management system you've learned and this sprint? Please explain why!
Redux is my favorite now that I kinda know what I am doing. It avoids prop drilling, makes our application code easy to follow and read and is an industry standard now with React. 
